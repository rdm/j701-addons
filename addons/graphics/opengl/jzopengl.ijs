...
if. IFUNIX do.
  ...
  hwnd=. gdk_win32_drawable_get_handle_jgtk_`gdk_win32_window_get_impl_hwnd_jgtk_@.(3=GTKVER_j_) window
...
if. ''-:y do.
  ...
  if. 0~: ctx=. getglctx area=. y do.
    ...
    if. IFUNIX do.
      ...
      if. OsMesa do.
        window=. getGtkWidgetWindow_jgtk_ area
        hwnd=. gdk_win32_drawable_get_handle_jgtk_`gdk_win32_window_get_impl_hwnd_jgtk_@.(2=GTKVER_j_) window
        hdc0=. GetDC hwnd
        BitBlt hdc0, xywh, hdc, 0, 0, SRCCOPY
        ReleaseDC hwnd, hdc0
      else.
