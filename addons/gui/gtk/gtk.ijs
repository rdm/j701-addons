...
if. 3=GTKVER_j_ do.
  ...
  case. 'Darwin' do.
    ...
    libigemac=: <dquote libdir,'libgtkmacintegration.2.dylib'
...
: win32
gdk_win32_drawable_get_handle > x x     : gtk2
gdk_win32_window_get_impl_hwnd > x x    : gtk3
: colormap
...
if. UNAME-:'Darwin' do.
  ...
  if. 3=GTKVER_j_ do. p=. jpath '~install/gtk3' else. p=. jpath '~install/gtk' end.
  f 'XDG_CONFIG_HOME';(p,'/etc');1
...
focus_in_event=: 3 : 0
if. 0<#'libgtk'ss >libigemac do.
  ((>libigemac),' gtk_mac_menu_set_menu_bar >n x')cd menu_bar
else.
